...
| 3 | Huly resource APIs | watchog exposes more read-only Huly data through the same cached-snapshot pull API: a project catalog (`/projects`), project detail (`/projects/{id}`), and project team (`/projects/{id}/team`). Members/owners resolve to the same person id + email fields the Summary API uses, so consumers map people on one id across both APIs | in-progress | `.claude/plans/huly-api-services.plan.md` |
| 4 | Orthogonal view APIs | watchog separates the data into orthogonal views derived from the same snapshots: a person directory (`/persons`, `/persons/{id}`), a per-person summary (`/persons/{id}/summary`), an overall summary (`/overview`), and a per-project summary (`/projects/{id}/summary`). No new Huly calls; existing routes unchanged | in-progress | `.claude/plans/huly-views-apis.plan.md` |
