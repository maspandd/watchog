...
//   loginEmailById:   Map<personRef, email>          (optional; login/account email, null when absent)
//   contactEmailById: Map<personRef, email>          (optional; contact-channel email, null when absent)
//
...
export function aggregate(
  ...
  for (const issue of issues) {
    ...
    if (row === undefined) {
      const loginEmail = loginEmailById.get(issue.assignee) ?? null
      const contactEmail = contactEmailById.get(issue.assignee) ?? null
      row = {
        id: issue.assignee,
