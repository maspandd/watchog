...
export async function fetchData(client) {
  ...
  // Project members/owners are stored as account UUIDs (Person.personUuid), a
  // different id space than Person._id. This map bridges them so project
  // membership resolves to the same person ids the summary uses.
  const personByUuid = new Map()
  for (const p of persons) {
    personNameById.set(p._id, nameOf(p))
    if (p.personUuid) personByUuid.set(p.personUuid, p._id)
  }
...
// Fetch the project catalog. Read-only, best effort: returns the raw projects
// flattened to plain data, or [] if the lookup fails. members[]/owners[] are
// left as account UUIDs here — resolution to people is the pure builder's job
// (lib/projects.mjs), which has the personByUuid map.
export async function fetchProjects(client) {
  try {
    const projects = await client.findAll(PROJECT_CLASS, {})
    return projects.map((pr) => ({
      id: pr._id,
      name: pr.name ?? String(pr._id),
      identifier: pr.identifier ?? null,
      description: pr.description ?? '',
      private: pr.private ?? false,
      archived: pr.archived ?? false,
      members: Array.isArray(pr.members) ? pr.members : [],
      owners: Array.isArray(pr.owners) ? pr.owners : [],
    }))
  } catch (err) {
    console.error('Project catalog fetch failed (projects resource will be empty):', err)
    return []
  }
}
